...
document.getElementById('encrypt').addEventListener('click', async () => {
    const plaintext = document.getElementById('plaintext').value;  // 获取明文输入框的值
    const result = await encrypt(plaintext);  // 调用加密函数并等待结果
    document.getElementById('ciphertext').value = result;  // 将加密结果显示在密文输入框中
});

document.getElementById('decrypt').addEventListener('click', async () => {
    const ciphertext = document.getElementById('ciphertext').value;  // 获取密文输入框的值
    const result = await decrypt(ciphertext);  // 调用解密函数并等待结果
    document.getElementById('plaintext').value = result;  // 将解密结果显示在明文输入框中
});
